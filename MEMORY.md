...
- [2026-09-19] **改 `packages/ui` 源码 dev 不生效**：vite root 是 `/app/apps/site`，不在 root 内的 workspace 包不被 watch → 已缓存的 `?t=` 模块图一直发旧代码（现象：改了组件、浏览器行为没变，极易误判"修复无效"）。验证/生效手段：`docker compose restart site`（或用全新路由文件强制新模块图）
- [2026-09-19] MetadataList（packages/ui）：`children` 在组件体里 materialize（为 toArray 计数）→ **早于 Context.Provider 实例化 MetadataListItem** → 条目 useContext 拿不到 labelConfig/orientation → columns={2}/"multi"（以及 horizontal 强制顶标、label top）看起来"失效"（dl 已是多列 grid，条目却仍是一行一个「标签|值」）。修复：把条目区拆成 Provider 内的子组件（MetadataListItems）再 materialize
- [2026-09-19] lazy prop（`title={<div/>}`）**用 Show 做存在性判断**会再 materialize 一次：SSR 在「元素创建」时分配 hydration key、客户端在「插入」时分配 → key 错位 → Hydration Mismatch（§10 的 children() 包装不足以解决）。正解：只读一次且在插入位置读，空态用 CSS `:empty` 收容器（MetadataList 的 title 已按此修）
