...
| 生产 | `master` | `api.dailog.fm` | `app.dailog.fm` | `dailog.fm` | Railway Production 环境 |
...
| `APP_ORIGINS` | `https://app.candelbot.app,https://candelbot.app,https://dailog.pages.dev,http://localhost:5173,http://localhost:3000`（每次新增前端域名都要补） | `https://app.dailog.fm,https://dailog.fm` |
...
| 项目 `dailog-studio` | production 环境 | preview 环境 |
|---|---|---|
...
| 自定义域名 | `app.dailog.fm`（待定） | `app.candelbot.app`（绑 dev 分支；要求域名在 CF DNS，否则用 branch alias `dev.dailog-studio.pages.dev`） |
...
| 变量 | `DATABASE_URL`=生产 Postgres、`VITE_API_BASE_URL`=`https://api.dailog.fm`、`VITE_SITE_BASE_URL`=`https://dailog.fm`、`VITE_STUDIO_BASE_URL`=`https://app.dailog.fm` | `DATABASE_URL`=dev Postgres、`VITE_API_BASE_URL`=`https://api.candelbot.app`、`VITE_SITE_BASE_URL`=`https://candelbot.app`、`VITE_STUDIO_BASE_URL`=`https://app.candelbot.app` |

> 变量（各环境）：`DATABASE_URL`（对应环境 Postgres，只读连接可加 `?sslmode=require`）、`VITE_API_BASE_URL`（`https://api.candelbot.app` / `https://api.dailog.fm`）、`VITE_SITE_BASE_URL`（站点自身，**登录代理以它作为 Origin 转发给 API**）、`VITE_STUDIO_BASE_URL`（`app.*`）。
> **site 与 studio 统一用 VITE_ 一套变量**——vite 构建期注入：客户端直接读；服务端（代理转发）的 `import.meta.env.VITE_*` 同样被替换成字面量，workerd 运行时无需环境变量。旧部署的 `API_BASE_URL`/`SITE_BASE_URL` 仍可兜底（代码兼容），新部署统一 VITE_。
...
- `app.candelbot.app` → CNAME `dailog-studio-dev.pages.dev`
