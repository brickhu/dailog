...
> 架构：site=`dailog.fm`（CF Pages）· studio=`app.dailog.fm`（CF Pages）· api=`api.dailog.fm`（Railway）
...
- [ ] 记录：
  ...
  | CNAME | app | studio 的 pages.dev 地址 |
...
| `APP_ORIGINS` | `https://dailog.fm,https://app.dailog.fm` |
...
- [ ] studio 项目：production branch = `master`；custom domain：`app.dailog.fm`
...
- [ ] 生成管理员 + 首批邀请码：本地连生产库跑 `pnpm invites:create <code> [--expires <天数>]`（DATABASE_URL 指向生产库，临时用）
- [ ] 验证链路（自查清单）：
  - [ ] `https://api.dailog.fm/health` → `{"ok":true}`
  - [ ] `dailog.fm` 打开正常、登录页 brand 显示 "dailog"
  - [ ] 注册 → 收到 `no-reply@dailog.fm` 验证邮件 → 点链接自动登录
  - [ ] `app.dailog.fm` SSO 免登录（跨子域 cookie）
  - [ ] 开通频道（邀请码）→ 录音 → 生成 → `dailog.fm/<slug>` 播放 + RSS
