...
## 实测状态更新（2026-08-03，用户决策）

浏览器登录态实测**按决策跳过**：子代理浏览器访问受限，且用户已确认 DOM 可提取对话信息（真实浏览器渲染分享页已实证完整对话可达）。本文选择器/机制以**交叉验证的公开逆向资料**为准，标注的"待补测"项在**各平台 content script 开发时逐一实测修正**（每平台适配器交付即验证），不阻塞扩展架构设计。
