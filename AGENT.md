...
- 导入：**仅 URL 分享链接 + 验证码验证**（先发码 → 再分享 → 采集校验）；**双路径采集**（API 快路径 + 浏览器扩展慢路径——无头浏览器已实测被 Turnstile 拦截，见 `docs/spikes/headless-cf.md`）；首发平台 ChatGPT/Claude/豆包/DeepSeek；无文本粘贴/导出通道
...
- [ ] M3a：快路径 api-fetcher 逐平台交付（DeepSeek 首发）
- [ ] M3b：浏览器扩展（慢路径，Claude 首发）——Manifest V3 + 按平台 content script + 商店上架
