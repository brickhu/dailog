...
**dailog**：一档由真实 AI 对话剪辑的访谈式播客——任何人（dailog.fm）投稿「分享链接 + 人设（主持人信息 + 声音采样）」，编辑部（admin.dailog.fm 管理员工作台，admin/editor）**人工驱动**大模型审核、润色、选脚本（可手工修改）、生成语音并自动发布（投稿人 = 主持人克隆音色，AI = 嘉宾品牌声线），在 dailog 单一品牌频道分发（播放页 + 单 feed RSS）。
