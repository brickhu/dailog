...
export interface ApiClient {
  /** timeoutMs：默认 30s；长任务（LLM 审核润色/发布预填）显式放宽 */
  get<T>(path: string, o?: { timeoutMs?: number }): Promise<T>;
  post<T>(path: string, body?: unknown, o?: { timeoutMs?: number }): Promise<T>;
  put<T>(path: string, body?: unknown, o?: { timeoutMs?: number }): Promise<T>;
  patch<T>(path: string, body?: unknown, o?: { timeoutMs?: number }): Promise<T>;
...
export function createApiClient(opts: ApiClientOptions): ApiClient {
  ...
  const json = async <T>(path: string, init?: RequestInit & { timeoutMs?: number }): Promise<T> => {
    const res = await request(path, init);
    return (await res.json()) as T;
  };

  // timeoutMs：默认 30s；长任务（LLM 审核润色/发布预填）调用方显式放宽（process 120s）
  const optsOf = (o?: { timeoutMs?: number }) => (o?.timeoutMs !== undefined ? { timeoutMs: o.timeoutMs } : {});
  return {
    get: <T>(path: string, o?: { timeoutMs?: number }) => json<T>(path, optsOf(o)),
    post: <T>(path: string, body?: unknown, o?: { timeoutMs?: number }) =>
      json<T>(path, { method: "POST", body: body === undefined ? undefined : serializeBody(body), ...optsOf(o) }),
    put: <T>(path: string, body?: unknown, o?: { timeoutMs?: number }) =>
      json<T>(path, { method: "PUT", body: body === undefined ? undefined : serializeBody(body), ...optsOf(o) }),
    patch: <T>(path: string, body?: unknown, o?: { timeoutMs?: number }) =>
      json<T>(path, { method: "PATCH", body: body === undefined ? undefined : serializeBody(body), ...optsOf(o) }),
    request,
  };
}
