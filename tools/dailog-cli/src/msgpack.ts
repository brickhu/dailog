/**
 * msgpack 最小编码器（覆盖 Fish TTS 所需类型：map/str/bin/array/int/bool/nil）。
 *
 * 从 services/api/src/tts/msgpack.ts 原样移植——该实现已对 api.fish.audio 实测通过
 * （服务器对 msgpack body 严格校验，编码错误返回 400；短字符串分支修过丢字节 bug，勿改动）。
 */
