...
describe.skipIf(!hasDb)("drizzle repo (integration, local PG)", () => {
  ...
  describe("episodes public audio", () => {
    it("getPublicAudioKey 仅已发布且公开的节目可读（音轨最新一条）", async () => {
      ...
      // 发布后（isPublic 置 true）→ 返回最新音轨 key + version（ETag）
      ...
      const pub = await repo.episodes.getPublicAudioKey(episodeId);
      expect(pub?.audioKey).toBe("episodes/k2.mp3");
  ...
  describe("profile persona", () => {
    it("updatePersona 保存/读取（JSONB 往返；null 清除）", async () => {
      const persona = { callName: "小明", gender: "男", profession: "程序员", age: "28", traits: "风趣幽默，雷厉风行" };
